fn main(){

    /*
    // let _number_64 = 4.0;      // compiler infers the value to use the default type f64
    // let _number_32: f32 = 5.0; // type f32 specified via annotation

    // Addition, Subtraction, and Multiplication
    // sonni opturib srazi type berib urvorsa bo'lardkan :)
    println!("1 + 2 = {} and 8 - 5 = {} and 15 * 3 = {}", 1u32 + 2, 8i32 - 5, 15 * 3);

    // Integer and Floating point division
    // Rust vashsheyakanueeee 
    println!("9 / 2 = {} but 9.0 / 2.0 = {}", 9u32 / 2, 9.0 / 2.0);
*/

/* 
    // boolean
    let is_bigger = 1 > 2;
    println!("Is 1 > 2 = {}", is_bigger);
*/

    // Char
    let thechar_uppercase = 'D';
    let thechar_lowercase = 'd';
    let thechar_smile_face = '😄';

    println!("D = {}", thechar_uppercase);
    println!("d = {}", thechar_lowercase);
    println!("smile = {}", thechar_smile_face);

    /*
    // output
    D = D
    d = d
    smile = 😄  // smileni poddeshka qibtasho'radikan 
 */


}